...
@Component({
  selector: 'app-ib-summary-serial-number',
  templateUrl: './ib-summary-serial-number.component.html',
  styleUrls: ['./ib-summary-serial-number.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class IbSummarySerialNumberComponent implements OnInit {
  ...
  */
  updateDataOnSearchAndLocateSort(sortColObj: any) {
    ...
    if (this.gridOptions.api) {
      this.gridOptions.api.setRowData(this.serialNumberData);
    }
  ...
  getSerialNumberData() {
    ...
    this.ibSummaryService.ibSummaryDataEmitter.subscribe(data => {
      ...
      if (this.serialNumberData && this.serialNumberData.length > 0) {
        this.showGrid = true;
    ...
    this.ibSummaryService.searchAndLocateDataEmitter.subscribe(data => {
      ...
      if (this.serialNumberData.length > 0) {
        this.showGrid = true;
      } else {
        this.showGrid = false;
      }
      ...
      */
        if (this.ibSummaryService.isSearchAndLocate) {
          ...
          this.ibSummaryService.updateRowDataOnSort(
            this.gridOptions.api.getSortModel(),
            IbSummaryService.SERIAL_NUMBER
          );
        }
      };
    } catch (error) {
  ...
  getSerialNumberOutsideData() {
    this.serialNumberService.getSerialNumberOutsideData().subscribe(data => {
      this.serialNumberOutside = data;
    });
  }

  getSerialNoMatchData() {
    this.serialNumberService.getSerialNoMatchData().subscribe(data => {
      this.noMatch = data;
    });
  }
  ...
  private createColumnDefs() {
    ...
    const data = this.appDataService.getDetailsMetaData(this.SERIAL_NUMBER);
  ...
  openSearchLocate() {
    ...
    this.searchLocateService.searchSerial = true;

import { Component, OnInit, ViewEncapsulation, Input, Renderer2, OnDestroy } from "@angular/core";
...
import { SuitesBreakdownService } from "./suites-breakdown.service";
...
@Component({
  selector: 'app-suites-breakdown',
  templateUrl: './suites-breakdown.component.html',
  styleUrls: ['./suites-breakdown.component.scss']
})
export class SuitesBreakdownComponent implements OnInit, OnDestroy {
    ...
    constructor(
      ...
      public suitesBreakdownService: SuitesBreakdownService,
    ...
    getSuitesData() {
      this.suitesBreakdownService
        .getSuitesData(this.archName)
    ...
    prepareSuitesMetaData(architectureMetaData: any) {
      ...
      for (let i = 0; i < architectureMetaData.length; i++) {
        ...
        this.suitesBreakdownService.namePersistanceColumnMap.set(
    ...
    private createColumnDefs() {
      ...
      if(this.appDataService.archName === 'SEC'){
        data = this.appDataService.getDetailsMetaData("PROSPECT_SUITE_SEC");
      } else {
        data = this.appDataService.getDetailsMetaData("PROSPECT_SUITE");
      }

...
import { SuitesCellComponent } from './suites-cell/suites-cell.component';
...
import { SuitesHeaderRenderComponent } from './suites-header-render/suites-header-render.component';
import { ProjectStoreService } from 'vnext/project/project-store.service';
import { DataIdConstantsService } from 'vnext/commons/services/data-id-constants.service';
import { TcvCellRendererComponent } from './tcv-cell-renderer/tcv-cell-renderer.component';
import { MigrateEligibleReasonsComponent } from 'vnext/modals/migrate-eligible-reasons/migrate-eligible-reasons.component';
import { ElementIdConstantsService } from 'vnext/commons/services/element-id-constants.service';
import { CreateTcoComponent } from 'vnext/modals/create-tco/create-tco.component';
import { TcoStoreService } from 'vnext/tco/tco-store.service';
import { PriceValidationComponent } from 'vnext/modals/price-validation/price-validation.component';
import { DownloadPricingDetailsComponent } from 'vnext/modals/download-pricing-details/download-pricing-details.component';
@Component({
...
export class PriceEstimationComponent implements OnInit, OnDestroy {
  ...
  readonly CONST_APPLICATIONS_INFRA_ENROLLMENT = 2;
  ...
  constructor(public priceEstimateService: PriceEstimateService, private utilitiesService: UtilitiesService, public vnextService: VnextService, public renderer: Renderer2, public proposalService: ProposalService,
    ...
    public vnextStoreService: VnextStoreService, public eaStoreService: EaStoreService, private activatedRoute: ActivatedRoute, private priceEstimationPollerService: PriceEstimationPollerService,
    private blockUiService: BlockUiService, private constantsService: ConstantsService,public localizationService:LocalizationService, public eaService: EaService, public projectStoreService: ProjectStoreService, public elementIdConstantsService: ElementIdConstantsService,
    private eaRestService: EaRestService, private tcoStoreService: TcoStoreService
) {
    ...
    this.gridOptions.frameworkComponents = {
      suitesRender: <{ new(): SuitesCellComponent }>(
        SuitesCellComponent
      ),
  ...
  ngOnInit() {
    ...
    if(this.punchInEnrollmentId) {
      ...
      const url  = sessionStorage.getItem('redirectOWB');
      if(url){
        this.redirectOWB = url.replace(/^"(.*)"$/, '$1')
      }     
    }
    ...
    if(!this.proposalStoreService.proposalData.dealInfo?.partnerDeal && this.isPartnerLoggedIn){
      this.proposalService.isReadOnlyProposal(false);
    }
    this.getEnrollmentData(true);
    ...
    this.subscribers.updateQtySubject = this.priceEstimateService.updateQtySubject.subscribe((response: any) => {
      if(response) {
    ...
    this.subscribers.updateTierForAtoSubject = this.priceEstimateService.updateTierForAtoSubject.subscribe((response: any) => {  
      this.priceEstimateService.updateTireMap(response);
      this.gridOptions.api.redrawRows();
    });
    this.subscribers.refreshPeGridData = this.priceEstimateService.refreshPeGridData.subscribe(() => {
   ...
   this.subscribers.addSuiteEmitter = this.priceEstimateService.addSuiteEmitter.subscribe((request) => {
      this.invokePollerForAddSuites = true;
    ...
    this.subscribers.updateProposalDataForCx = this.priceEstimateService.updateProposalDataForCx.subscribe((data:any) => {
      if(this.eaService.features.FIRESTORM_REL){// here data will an obj having proposalData and enrollment
        if(data.proposalData){
          // check and set syncStatus
          if(data.proposalData?.syncStatus){
            this.proposalStoreService.proposalData.syncStatus = data.proposalData.syncStatus;
          }
          this.setErrorMessage(data.proposalData, true);
          this.setTotalValues(data.proposalData);
        }
        if(data.swEnrollmentData && data.swEnrollmentData[0]?.embeddedHwSupportAttached){
          this.setGridData(data.swEnrollmentData);
        }
      } else {// here data is proposal data.
        this.setErrorMessage(data, true);
        this.setTotalValues(data);
      }
    });
    ...
    if(this.eaService.features.STO_REL && !this.isChangeSubFlow && this.proposalStoreService.proposalData.status === 'IN_PROGRESS' && (!this.proposalStoreService.proposalData.scopeInfo.returningCustomer || (this.proposalStoreService.proposalData.scopeInfo?.returningCustomer && this.proposalStoreService.proposalData?.scopeInfo?.newEaidCreated))){
      this.checkForSto();
    }
  }
  ...
  checkForSto(){
    ...
    this.eaRestService.getApiCall(url).subscribe((response: any) => {
      if(this.vnextService.isValidResponseWithoutData(response)){
        if(response.data?.strategicOffers && response.data?.strategicOffers.length){
          this.isStrategicOffer = true;
        }
      }
    })
  }
  ...
  openEAMS() {
    ...
    modal.result.then((result) => {
      if (result.continue === true) {
        this.showEAMS = false;
        if (this.priceEstimateStoreService.displayCxGrid && this.priceEstimateStoreService.selectedEnrollment.cxAttached){
     ...
     if(enrollment.id){
      this.proposalRestService.postApiCall(url,requestJson).subscribe((response: any) => {
        if(this.vnextService.isValidResponseWithData(response)) { 
         ...
         // check and set isSplunkSuitesAdded
         if(enrollment.id === this.CONST_APPLICATIONS_INFRA_ENROLLMENT && this.eaService.features.SPLUNK_SUITE_REL && response.data?.proposal){
          this.proposalStoreService.proposalData.isSplunkSuitesAdded = response.data.proposal?.isSplunkSuitesAdded ? true : false;
         }
        ...
        if(enrollment.id){
          this.proposalRestService.deleteApiCall(url).subscribe((response: any) => {
            if (this.vnextService.isValidResponseWithoutData(response)) {
              ...
              // check and set isSplunkSuitesAdded
              if(enrollment.id === this.CONST_APPLICATIONS_INFRA_ENROLLMENT && this.eaService.features.SPLUNK_SUITE_REL && this.proposalStoreService.proposalData.isSplunkSuitesAdded){
                this.proposalStoreService.proposalData.isSplunkSuitesAdded = false;
              }
              ...
              if (this.priceEstimateStoreService.viewAllSelected && this.selectedEnrollmentArray.length > 1) {
                ...
                // response.data.enrollments.filter(enrollment => enrollment.id === this.priceEstimateStoreService.selectedEnrollment.id);
                // this.setGridData(response.data);
  ...
  setGridData(data) { 
    ...
    this.priceEstimateService.isMigrationSuitesPresentInEnrollment = false; // reset flag
   ...
   // this.onGridReady('');
    ...
    // }
    if (((data.enrollments[0].awaitingResponse && !this.eaService.features.IB_OPTIMIZATION) || (this.eaService.features.IB_OPTIMIZATION && this.proposalStoreService.proposalData?.syncStatus?.cxCcwrRipFlag)) || (data.enrollments[0].embeddedHwSupportAttached && !data.enrollments[0].cxAttached && this.eaService.features.FIRESTORM_REL && (((this.priceEstimateStoreService.selectedCxEnrollment?.awaitingResponse && !this.eaService.features.IB_OPTIMIZATION) || (this.eaService.features.IB_OPTIMIZATION && this.proposalStoreService.proposalData?.syncStatus?.cxCcwrRipFlag)) || (((!this.priceEstimateStoreService.selectedCxEnrollment?.awaitingResponse && !this.eaService.features.IB_OPTIMIZATION) || (this.eaService.features.IB_OPTIMIZATION && !this.proposalStoreService.proposalData?.syncStatus?.cxCcwrRipFlag)) && this.invokePollerForAddSuites)))) {
      this.invokePollerForAddSuites = false; // reset this flag once called poller service after above condtions met
      this.priceEstimationPollerService.stopPollerService();
      this.invokePollerService();
    }  
    if (this.eaService.features.FIRESTORM_REL && data.enrollments[0].embeddedHwSupportAttached) {
     this.isDnxSuiteAdded = true;
    }
    setTimeout(() => {
      this.gridOptions?.api?.forEachNode(node => {
        ...
        if (this.priceEstimateService.swExpandedSuiteArr.length) {
          this.priceEstimateService.swExpandedSuiteArr.forEach((suite) => {
            if (node.data.poolSuiteLineName === suite) {
              node.setExpanded(true);
            }
          });
        }
  ...
  getEnrollmentData(pageload=false, fromProposalUpdate?) {
    ...
    this.proposalRestService.getApiCall(url).subscribe((response: any) => {
        if(this.vnextService.isValidResponseWithData(response)){
          this.setEnrollmentData(response, fromProposalUpdate);
          ...
          // check and set isSplunkSuitesAdded
          if(this.eaService.features.SPLUNK_SUITE_REL && response.data?.proposal && !this.proposalStoreService.proposalData.isSplunkSuitesAdded){
            this.proposalStoreService.proposalData.isSplunkSuitesAdded = response.data.proposal?.isSplunkSuitesAdded ? true : false;
          }
  ...
  checkForDisablingHybrid() {
    ...
    if(!hybridEnrollment || hybridEnrollment?.alreadyPurchased){
      return;
    }
  ...
  setEnrollmentData(response, fromProposalUpdate?) {
    this.selectedEnrollmentArray = [];
    ...
    if(this.eaStoreService.changeSubFlow){
      ...
      this.proposalStoreService.proposalData.enrollments.forEach((enrollment) => {
        if(this.eaService.features.CROSS_SUITE_MIGRATION_REL){
          if(enrollment.suitesNotAvailableForMigration){
            this.displayMigrationReasonMsg = true
          }
        }
    ...
    // emit enrollments data when proposal updated from edit window
    if(fromProposalUpdate && this.eaService.features?.COTERM_SEPT_REL){
      this.priceEstimateService.enrollmentUpdateSubject.next(response.data.enrollments);
    }
    setTimeout(() => {
  ...
  // method to check eligible enrollment to load first
  eligibleForSelectedEnrollment(enrollment) {
    let isEligibleForSelectedEnrollment = false;
    if (this.eaService.features.CROSS_SUITE_MIGRATION_REL && this.isUpgradeFlow) {
      isEligibleForSelectedEnrollment = (!enrollment.enrolled && enrollment?.includedInChangeSub && this.isChangeSubFlow && !(enrollment.id === 4 || enrollment.id === 6) && (enrollment?.eligibleForMigration || enrollment?.eligibleForUpgrade));
    } else {
      isEligibleForSelectedEnrollment = (!enrollment.enrolled && enrollment?.includedInChangeSub && this.isChangeSubFlow && !(enrollment.id === 4 || enrollment.id === 6))
    }
    return isEligibleForSelectedEnrollment;
  }
  ...
  setSelectedEnrollmentData() { 
    ...
    //To show View Document Center Lable on the sub header components
      ...
      if (!enrollment.disabled && (enrollment.enrolled || this.eligibleForSelectedEnrollment(enrollment))){
        if(enrollment.id === 6){
          this.priceEstimateStoreService.isHybridEnrollmentSelected = true;
        }
        this.selectedEnrollmentArray.push(enrollment);
  ...
  setTotalValues(proposalData:IProposalInfo){
    if(proposalData){     
      ...
      this.enableContinue = proposalData.allowCompletion; 
        if(this.proposalStoreService.proposalData.status !== 'COMPLETE' && (this.eaService.isResellerLoggedIn || (this.eaService.isDistiLoggedIn && this.eaService.isResellerOpty && this.proposalStoreService.proposalData && !this.proposalStoreService.proposalData.sharedWithDistributor)) && this.proposalStoreService.proposalData.dealInfo?.partnerDeal){
  ...
  //  this.blockUiService.spinnerConfig.noProgressBar();
    if(this.priceEstimateStoreService.selectedEnrollment?.id){
      this.pollerSubscibers = this.priceEstimationPollerService.invokeGetPollerservice(this.getPollerServiceUrl()).subscribe((res: any) => {
        // check and set syncStatus
        if(res.data && res.data.proposal && res.data.proposal?.syncStatus){
        ...
        if(this.eaService.features.FIRESTORM_REL){
          ...
          if (res.data && res.data.enrollments && res.data.enrollments.length) {
            ...
            if(cxEnrollment){
              ...
              const data = {proposal:res.data.proposal,enrollments: [swEnrollment]}
    ...
    this.columnDefs = [
      {
        ...
        "width": 680,
        "headerComponentFramework": <{ new(): SuitesHeaderRenderComponent }>(
          SuitesHeaderRenderComponent),
        "cellRendererParams": {
      ...
      // },
      {
        ...
        "headerComponentParams": {
          template:
              ...
              '  <span class="i-vNext-square-arrow" action-id="pe.icon.breakUp"><span class="path1"></span><span class="path2"></span><span class="path3 softwarePaBreakup"></span></span> <span ref="eText" class="ag-header-cell-text" role="columnheader"></span>' +
              '  </div>' +
              '</div>'
        },
        "width": 125,
      ...
      },
      {
        ...
        "headerComponentParams": {
          template:
              '<div class="ag-cell-label-container" role="presentation">' +
              '  <span class="i-vNext-square-arrow" action-id="pe.icon.breakUp"><span class="path1"></span><span class="path2"></span><span class="path3 tcvBreakup"></span></span> <span ref="eText" class="ag-header-cell-text" role="columnheader"></span>' +
              '  </div>' +
              '</div>'
        },
        "width": 140,
  ...
  onCellClicked($event) {
   ...
   if(this.gridOptions?.api){
    this.gridOptions.api.forEachNode(node => {
      if (node.expanded) {
        this.priceEstimateService.swExpandedSuiteArr.push(node.data.poolSuiteLineName);
      } else {
        for (let i = 0; i < this.priceEstimateService.swExpandedSuiteArr.length; i++) {
          if (node.data.poolSuiteLineName === this.priceEstimateService.swExpandedSuiteArr[i]) {
            this.priceEstimateService.swExpandedSuiteArr.splice(node.data.poolSuiteLineName)
          }
        }
      }
    });
 ...
 //Update suite count in enrollment 
  ...
  ngOnDestroy(){
    ...
    this.priceEstimateService.swExpandedSuiteArr = []
    ...
    if (this.subscribers.updateQtySubject){
      this.subscribers.updateQtySubject.unsubscribe();
    }
    if (this.subscribers.updateTierForAtoSubject){
      this.subscribers.updateTierForAtoSubject.unsubscribe();
    }
    if (this.subscribers.refreshPeGridData){
      this.subscribers.refreshPeGridData.unsubscribe();
    }
    this.priceEstimateService.isMigrationSuitesPresentInEnrollment = false;
  }
  ...
  closeEditProposalParam(event){
    if(event){
      this.getEnrollmentData(false, true);//reset all data on PE page;
      if(this.priceEstimateStoreService.displayCxGrid && this.priceEstimateStoreService.selectedEnrollment.cxAttached) {
  ...
  openUpgradeSummary(){
    const url = this.vnextService.getAppDomainWithContext + 'proposal/'+ this.proposalStoreService.proposalData.objId + '/enrollment';
      this.proposalRestService.getApiCall(url).subscribe((response: any) => {
        ...
        if(this.eaService.features.CROSS_SUITE_MIGRATION_REL){
          this.upgradeSummaryData = this.priceEstimateService.prepareMigrationUpgradeSummarydata(response.data);
        } else {
          this.upgradeSummaryData = this.priceEstimateService.prepareUpgradeSummaryData(response.data);
        }
          if (response.data.enrollments){
            const modal = this.modalVar.open(UpgradeSummaryComponent, { windowClass: 'lg-width-modal', backdropClass: 'modal-backdrop-vNext' });
  ...
  downloadExcel() {
    ...
    if (this.eaService.features.SERVICE_LED) {
      const modal = this.modalVar.open(DownloadPricingDetailsComponent, { windowClass: 'download-pricing-modal', backdropClass: 'modal-backdrop-vNext' });
      modal.result.then((result)=> {
  ...
  openOfferModal() {
    if(this.proposalStoreService.projectStatus === 'COMPLETE' && !this.proposalStoreService.isReadOnly){
      this.priceEstimateStoreService.showStrategicOffers = true;
      this.renderer.addClass(document.body, 'position-fixed');
    }
 }
  closeIbPullMsg() {
      ...
      if(this.eaService.features.FIRESTORM_REL){
        ...
        if (this.priceEstimateStoreService.selectedEnrollment.cxAttached) {
          if(cxEnrollment){
            ...
            this.priceEstimateService.updateCxGridforIbRepullSubject.next(data)
          }
        } else {
          const enrollmentData = cxEnrollment;
          ...
          this.priceEstimateStoreService.displayIbPullMsg = (cxEnrollment.systematicIbRepullRequired &&  this.priceEstimateStoreService.diffInDaysForSystematicIbRepull >= 10 && !this.proposalStoreService.isReadOnly) ? true : false;
        }
        if(swEnrollment && swEnrollment.embeddedHwSupportAttached){
          const data = {enrollments: [swEnrollment]}
          this.setGridData(data);
        }
      } else {
        this.setGridData(response.data);
...
showUnavailbaleMigrateReasons() {
  const modal = this.modalVar.open(MigrateEligibleReasonsComponent, { windowClass: 'migrate-upgrade-eligible', backdropClass: 'modal-backdrop-vNext' });
 ...
 invokePollerServiceForSwOnly(event){
  this.invokePollerService()
 }


 // to call initiate IB/ reprocess IB/ Update IB assessment 
 callIbReprocessOptimization(reprocessIb, lastIbAssessmentAction?, isTcoValidate=false){
  if((this.proposalStoreService.isPartnerAccessingSfdcDeal && !this.vnextStoreService.loccDetail?.loccSigned) || (lastIbAssessmentAction && this.proposalStoreService.proposalData?.syncStatus.cxCcwrRipFlag) || (reprocessIb && !this.checkIfLastRepullPassed24Hrs() && !isTcoValidate) ){
    return;
  }
  ...
  if (isTcoValidate) {
    reqObj.data['transactionType'] = 'REPRICE';
  }
...
checkIfLastRepullPassed24Hrs(){
  if (this.priceEstimateStoreService?.selectedCxEnrollment) {
    const lastRepullDate = this.priceEstimateStoreService.selectedCxEnrollment.hwLastIbPulledAtDate;
  ...
  // This function will call when user click on Create TCO button
  getTcoId() {
    if (this.proposalStoreService.proposalData?.syncStatus?.tcoIbReAssessmentRequired && this.proposalStoreService.proposalData?.status === "IN_PROGRESS") {
      this.openPriceValidationPopUp();
    } else {
      const url = this.vnextService.getAppDomainWithContext + 'proposal/' + this.proposalStoreService.proposalData.objId + '/tco';
      this.eaRestService.getApiCall(url).subscribe((response: any) => {
        if (this.vnextService.isValidResponseWithData(response)) {
          if (response.data?.tcoCreated && response.data?.tcoObjId) {
            this.router.navigate(['ea/project/proposal/' + this.proposalStoreService.proposalData.objId + '/tcodetails/' + response.data.tcoObjId]);
          } else {
            this.createTcoPopup();
          }
        }
  ...
  // method to open pricevalidation popup
  openPriceValidationPopUp(){
    const modal = this.modalVar.open(PriceValidationComponent, { windowClass: 'lg', backdropClass: 'modal-backdrop-vNext' });
    modal.result.then((result) => {
      if(result?.continue){
        // call api to reprocess-ib
        this.callIbReprocessOptimization(true, 'repullIb', true);
      }
    });
  }

  createTcoPopup(){
    const modal = this.modalVar.open(CreateTcoComponent, { windowClass: 'lg tco-modeling', backdropClass: 'modal-backdrop-vNext' });
    modal.result.then((result) => {
      if (result?.data) {
        ...
        this.eaRestService.postApiCall(url, request).subscribe((response: any) => {
          if (this.vnextService.isValidResponseWithData(response)) {
            ...
            this.proposalStoreService.tcoCreateFlow = true;
            this.router.navigate(['ea/project/proposal/' + this.proposalStoreService.proposalData.objId + '/tcodetails/' + this.tcoStoreService.tcoData.objId]);
          }
        })
      }
    });
  ...
  // createTCO() {
  //   if (this.proposalStoreService.proposalData.tcoObjId) {//if TCO is already created
  //     const url = this.vnextService.getAppDomainWithContext + 'proposal/tco/' + this.proposalStoreService.proposalData.tcoObjId;
  //     this.eaRestService.getApiCall(url).subscribe((response: any) => {
  //       if (this.vnextService.isValidResponseWithData(response)) {
  //         this.tcoStoreService.tcoData = response.data;
  //         this.router.navigate(['ea/project/proposal/' + this.proposalStoreService.proposalData.objId + '/tcodetails/' + this.tcoStoreService.tcoData.tcoObjId]);
  //       }
  //     })

  //   } else {// to create new TCO
  //     const modal = this.modalVar.open(CreateTcoComponent, { windowClass: 'lg tco-modeling', backdropClass: 'modal-backdrop-vNext' });
  //     modal.result.then((result) => {
  //       if (result?.data) {
  //         const request = result
